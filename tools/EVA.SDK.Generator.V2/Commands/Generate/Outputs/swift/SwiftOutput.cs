...
internal class SwiftOutput : IOutput<SwiftOptions>
{
  ...
  private void WriteType(TypeSpecification type, string id, string typename, IndentedStringBuilder output, OutputContext<SwiftOptions> ctx, bool isNested = false)
  {
    ...
    using (output.Indentation)
    {
      ...
      using (output.Indentation)
      {
        ...
        {
          var prop = list[i];
          var propDefault = GetPropDefault(prop.Value);
          output.WriteLine(
        ...
        if (prop.Type is { Name: ApiSpecConsts.Specials.Option, Arguments: var options })
        {
          ...
          using (output.Indentation)
          {
            ...
            using (output.Indentation)
            {
              ...
              {
                var option = list[i];
                var propDefault = GetTypeDefault(option);
                var typeName = GetTypeName(option, ctx);
                var name = option.Name.Replace("+", ".").Split(".").Last();
                output.WriteLine(
  ...
  {
    var typeName = GetCorePropTypeName(ps, name, ctx, forceNotNullable, forceNullable);
    return ps.Skippable ? $"Maybe<{typeName}>" : typeName;
  }

  private static string GetCorePropTypeName(PropertySpecification ps, string name, OutputContext<SwiftOptions> ctx, bool forceNotNullable = false, bool forceNullable = false)
  {
  ...
  private static string GetTypeDefault(TypeReference typeReference)
  {
    return typeReference.Nullable ? "nil" : string.Empty;
  }

  private static string GetPropDefault(PropertySpecification ps)
  {
    return ps.Skippable ? ".some(nil)" : GetTypeDefault(ps.Type);
  }
